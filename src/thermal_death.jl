...
"""
function accumulated_injury(m::LogLinearTDTModel, T_series, dt_minutes)
    increments = [_C(T) >= m.incipient_temperature ?
                  dt_minutes / survival_time(m, T) : 0.0
                  for T in T_series]
    cumsum(increments)
end
...
"""
function time_to_failure(m::LogLinearTDTModel, T_series, dt_minutes; critical_injury=1.0)
    injuries = accumulated_injury(m, T_series, dt_minutes)
